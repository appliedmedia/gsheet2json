...
const VERSION = "1.2.44"; // latest known version; build substitutes the exact package.json version into the bundle
...
function gsheet2json_handleCommand(cmd: string, arg?: any): any {
  const fns: Record<string, Function> = {
    ...
    getJsonFilesInDrive,
    loadDriveJson,
    driveChanges: (payload: { token: string }) => getDriveChanges(payload.token),
